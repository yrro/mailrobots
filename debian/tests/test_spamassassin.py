...
def test_spamd_receives_no_mail(sendmail, print_logs, print_journal):
    try:
        with raises(smtplib.SMTPRecipientsRefused) as excinfo:
            sendmail(To='spamd@spamd.invalid', Subject='spamd reject test', body='blah\r\n')
...
def test_spam_headers_added(sendmail, imap, print_logs, print_journal, header):
    try:
        sendmail(To='account@test.example', Subject='spam header test', headers={header: 'should be removed'}, body='A sample message.\r\n')
        imap.select()
